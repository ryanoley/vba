    ...
    For Each final_order_a In final_orders
        ...
        If trade_base_b Is Nothing Then
            ...
            With Sheets("FM Upload")
                ...
                .Range("F2:F3").Value = order_a_qty
                .Range("G2:G3").Value = order_a_qty
                .Range("H2:H3").Value = trade_a_ramid
                ' Handle situation where changing sides
                If Not (final_order_b Is Nothing) Then
                    .Range("E3").Value = final_order_b.Offset(0, col_ix_fo_ord_type)
                    .Range("F3").Value = final_order_b.Offset(0, col_ix_fo_qty)
                    .Range("G3").Value = final_order_b.Offset(0, col_ix_fo_qty)
                Else
                    .Rows(3).Delete
                End If
            End With
        ...
        ' Trading in both ERN and PEAD
        If Not (trade_base_b Is Nothing) Then
            ...
            If Sgn(trade_a_qty) = Sgn(trade_b_qty) Then
                ...
                With Sheets("FM Upload")
                    ...
                    .Range("F2").Value = trade_a_qty
                    .Range("F3").Value = trade_b_qty
                    .Range("G2").Value = trade_a_qty
                    .Range("G3").Value = trade_b_qty
                    .Range("H2").Value = trade_a_ramid
                    .Range("H3").Value = trade_b_ramid
                    '  Two trades and one order exist
                    If final_order_b Is Nothing Then
                        .Range("E2:E3").Value = order_a_ordtype
                    Else
                        ' Two trades and two orders exist
                        order_b_ordtype = final_order_b.Offset(0, col_ix_fo_ord_type)
                        If Abs(trade_b_qty) = order_a_qty Then
                            .Range("E2").Value = order_b_ordtype
                            .Range("E3").Value = order_a_ordtype
                            .Range("F2").Value = Abs(trade_b_qty)
                            .Range("F3").Value = Abs(trade_a_qty)
                            .Range("G2").Value = Abs(trade_b_qty)
                            .Range("G3").Value = Abs(trade_a_qty)
                            .Range("H2").Value = trade_b_ramid
                            .Range("H3").Value = trade_a_rami
                        Else
                ...
                End With
            Else
                ' Different sides - requires phantom trades
                With Sheets("FM Upload")
                    ...
                    If Abs(trade_a_qty) > Abs(trade_b_qty) Then
                        ...
                        If trade_b_qty > 0 Then
                            ph_ordtype_b = "Buy"
                        Else
                            ph_ordtype_b = "SellShort"
                        End If
                        .Range("C2:C3").Value = trade_a_tradeid
                    ...
                    Else
                        ...
                        If trade_a_qty > 0 Then
                            ph_ordtype_b = "BuyToCover"
                        Else
                            ph_ordtype_b = "Sell"
                        End If
